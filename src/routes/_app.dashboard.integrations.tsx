    ...
    <div className="space-y-6">
      <header className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <div className="flex gap-2">
          <button onClick={() => setAgencyKeyModal("dataforseo")} className="vt-btn-secondary text-xs flex items-center gap-1.5">
            <Key className="h-3.5 w-3.5" /> DataForSEO Key
          </button>
          <button onClick={() => setAgencyKeyModal("semrush")} className="vt-btn-secondary text-xs flex items-center gap-1.5">
      ...
      <div className="space-y-3">
        {rows.length === 0 && (
          <div className="vt-card p-8 text-center text-[color:var(--muted)]">No clients yet.</div>
        )}
        {rows.map((c) => <ClientCard key={c.id} c={c} />)}
      </div>

      <p className="text-xs text-[color:var(--muted)]">
        Credentials are stored encrypted. Toggle off to disable a source without removing it.
      </p>

      {agencyKeyModal && (
        <AgencyKeyModal provider={agencyKeyModal} onClose={() => setAgencyKeyModal(null)} />
      )}
    </div>
  );
}
...
function ClientCard({ c }: { c: any }) {
  ...
  return (
    <div className="vt-card overflow-hidden">
        ...
        <div className="flex items-center gap-3">
          <div className="flex gap-1.5">
            {PROVIDER_KEYS.map((p) => {
              ...
              const Icon = PROVIDER_ICONS[p];
              return (
                <div key={p} title={`${PROVIDER_LABELS[p]}: ${active ? "Active" : row?.has_credentials ? row.status : "Not connected"}`}
          ...
          <span className="text-xs text-[color:var(--muted)]">{c.connectedCount}/{PROVIDER_KEYS.length} active</span>
      ...
      {manualModal && (
        <ManualDataModal clientId={c.id} clientName={c.name} existing={c.providers.manual} onClose={() => setManualModal(false)} />
      )}
      {agencyKeyModal && (
        <AgencyKeyModal provider={agencyKeyModal} onClose={() => setAgencyKeyModal(null)} />
      )}
      {propertyModal && (
        <PropertySelectorModal clientId={c.id} clientName={c.name} onClose={() => setPropertyModal(false)} />
      )}
    </div>
  );
}

function ProviderRow({ clientId, clientName, provider, row, onManualEdit, onAddAgencyKey, onOpenPropertyModal }: {
  clientId: string;
  clientName: string;
  provider: Provider;
  row: any;
  ...
  return (
    <div className={`flex items-center gap-3 rounded-lg px-3 py-2.5 border ${
      ...
      has ? "bg-[color:var(--slate)]/20 border-[color:var(--border)] opacity-60" :
      "bg-[color:var(--slate)]/20 border-[color:var(--border)]"
    }`}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-xs text-[color:var(--muted)]">
          {has ? (active ? "Active — data will be used in audits" : "Disabled") : PROVIDER_DESCRIPTIONS[provider]}
        </div>
        {row?.last_error && <div className="text-[10px] text-amber-400 mt-0.5 truncate">{row.last_error}</div>}
      </div>

      <div className="flex items-center gap-2 shrink-0">
        {has ? (
          active ? <CheckCircle2 className="h-4 w-4 text-emerald-400" /> :
          <XCircle className="h-4 w-4 text-[color:var(--muted)]" />
        ) : (
          <XCircle className="h-4 w-4 text-[color:var(--muted)]/40" />
        )}

        {has && (
          <button onClick={handleToggle} disabled={busy === "toggle"} title={active ? "Disable" : "Enable"}>
            {active ? <ToggleRight className="h-5 w-5 text-emerald-400" /> : <ToggleLeft className="h-5 w-5 text-[color:var(--muted)]" />}
          </button>
        )}

        {provider === "manual" ? (
          <button onClick={onManualEdit} className="text-xs flex items-center gap-1 text-[color:var(--accent)] hover:underline">
            <Pencil className="h-3 w-3" /> {has ? "Edit" : "Add data"}
          </button>
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={async () => {
                ...
                const timer = setInterval(() => {
                  if (popup?.closed) {
                    clearInterval(timer);
                    setTimeout(() => { onOpenPropertyModal?.(); }, 800);
                  }
                }, 500);
              }}
            ...
            >
              ...
              {has ? "Reconnect" : `Connect ${provider === "google" ? "GA4" : "GSC"}`}
            </button>
            {has && (
              <>
                <button onClick={handleTest} disabled={!!busy} className="text-xs flex items-center gap-1 text-[color:var(--accent)] hover:underline disabled:opacity-50">
                  <RotateCw className={`h-3 w-3 ${busy === "test" ? "animate-spin" : ""}`} /> Test
                </button>
                <button onClick={handleDisconnect} disabled={!!busy} className="text-xs flex items-center gap-1 text-red-400 hover:underline disabled:opacity-50">
          ...
          onAddAgencyKey ? (
            <button onClick={() => onAddAgencyKey(provider)} className="text-xs flex items-center gap-1 text-[color:var(--accent)] hover:underline font-medium">
...
function PropertySelectorModal({ clientId, clientName, onClose }: { clientId: string; clientName: string; onClose: () => void }) {
  const qc = useQueryClient();
  ...
  useEffect(() => {
    getPropsFn({ data: { clientId } })
      .then((d) => { setProps(d); setSelectedGa4(d.ga4Selected ?? ""); setSelectedGsc(d.gscSelected ?? ""); })
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="vt-card w-full max-w-lg p-6 space-y-4">
        <div>
          ...
          <p className="text-sm text-[color:var(--muted)] mt-1">
            {props?.accountEmail ? `Connected as ${props.accountEmail}` : "Choose which GA4 property and GSC site to use for audits."}
          </p>
        </div>
          ...
          <div className="space-y-4">
            {props?.ga4Properties?.length > 0 ? (
                ...
                <div className="space-y-1.5 max-h-48 overflow-y-auto">
                  {props.ga4Properties.map((p: any) => (
                    <label key={p.propertyId} className={`flex items-center gap-3 px-3 py-2 rounded-lg cursor-pointer border transition-colors ${
                      selectedGa4 === p.propertyId ? "border-[color:var(--accent)] bg-[color:var(--accent)]/10" : "border-[color:var(--border)] hover:border-[color:var(--accent)]/50"
                    }`}>
                      <input type="radio" name="ga4" value={p.propertyId} checked={selectedGa4 === p.propertyId} onChange={() => setSelectedGa4(p.propertyId)} className="accent-[color:var(--accent)]" />
                      <div>
              ...
              <div className="text-sm text-amber-400 bg-amber-400/10 rounded-lg px-3 py-2">
                No GA4 properties found. Make sure the Google account has GA4 access, then reconnect.
              </div>
            )}
                ...
                <div className="space-y-1.5 max-h-36 overflow-y-auto">
                  {props.gscSites.map((site: string) => (
                    <label key={site} className={`flex items-center gap-3 px-3 py-2 rounded-lg cursor-pointer border transition-colors ${
                      selectedGsc === site ? "border-[color:var(--accent)] bg-[color:var(--accent)]/10" : "border-[color:var(--border)] hover:border-[color:var(--accent)]/50"
                    }`}>
                      <input type="radio" name="gsc" value={site} checked={selectedGsc === site} onChange={() => setSelectedGsc(site)} className="accent-[color:var(--accent)]" />
...
function ManualDataModal({ clientId, clientName, existing, onClose }: { clientId: string; clientName: string; existing: any; onClose: () => void }) {
  const qc = useQueryClient();
  const saveFn = useServerFn(saveManualData);
  const [saving, setSaving] = useState(false);
  const [form, setForm] = useState({ sessions: "", users: "", conversion_rate: "", bounce_rate: "", avg_order_value: "", organic_keywords: "", organic_traffic: "", clicks: "", impressions: "" });
  ...
  function field(key: keyof typeof form, label: string, hint?: string) {
    return (
      <div>
        ...
        <input type="number" placeholder={hint ?? "Optional"} className="vt-input" value={form[key]} onChange={(e) => setForm((f) => ({ ...f, [key]: e.target.value }))} />
      </div>
    );
  }
  ...
  async function handleSave() {
    ...
    try {
      ...
      await saveFn({ data: { clientId, data: { sessions: num(form.sessions), users: num(form.users), conversion_rate: num(form.conversion_rate), bounce_rate: num(form.bounce_rate), avg_order_value: num(form.avg_order_value), organic_keywords: num(form.organic_keywords), organic_traffic: num(form.organic_traffic), clicks: num(form.clicks), impressions: num(form.impressions) } } });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="vt-card w-full max-w-lg p-6 space-y-4">
        <div>
          ...
          <p className="text-sm text-[color:var(--muted)] mt-1">Enter analytics data manually. Leave fields blank to skip.</p>
        </div>
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="col-span-2 text-xs font-medium text-[color:var(--muted)] uppercase tracking-wide pt-2">Search Console</div>
          {field("clicks", "Clicks (30d)")}
          {field("impressions", "Impressions (30d)")}
          <div className="col-span-2 text-xs font-medium text-[color:var(--muted)] uppercase tracking-wide pt-2">Competitive</div>
          {field("organic_keywords", "Organic Keywords")}
          {field("organic_traffic", "Organic Traffic (monthly)")}
        </div>
        <div className="flex gap-3 pt-2">
          <button onClick={onClose} className="vt-btn-secondary flex-1">Cancel</button>
          <button onClick={handleSave} disabled={saving} className="vt-btn-primary flex-1">{saving ? "Saving…" : "Save Data"}</button>
        </div>
      </div>
    </div>
  );
...
function AgencyKeyModal({ provider, onClose }: { provider: "dataforseo" | "semrush"; onClose: () => void }) {
  const qc = useQueryClient();
  ...
  async function handleSave() {
    setSaving(true);
    try {
      const credentials = provider === "dataforseo" ? { login, password } : { apiKey };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="vt-card w-full max-w-md p-6 space-y-4">
        <div>
          ...
          <p className="text-sm text-[color:var(--muted)] mt-1">Shared across all clients as a fallback data source.</p>
        </div>
        {provider === "dataforseo" ? (
          <div className="space-y-3">
            <div><label className="block text-xs font-medium text-[color:var(--muted)] mb-1">Login (email)</label><input type="email" className="vt-input" value={login} onChange={(e) => setLogin(e.target.value)} placeholder="your@email.com" /></div>
            <div><label className="block text-xs font-medium text-[color:var(--muted)] mb-1">Password</label><input type="password" className="vt-input" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="DataForSEO API password" /></div>
            <p className="text-xs text-[color:var(--muted)]">Get credentials at <a href="https://app.dataforseo.com/api-dashboard" target="_blank" rel="noreferrer" className="vt-link">dataforseo.com</a></p>
          </div>
        ) : (
          <div className="space-y-3">
            <div><label className="block text-xs font-medium text-[color:var(--muted)] mb-1">API Key</label><input type="password" className="vt-input" value={apiKey} onChange={(e) => setApiKey(e.target.value)} placeholder="Semrush API key" /></div>
            <p className="text-xs text-[color:var(--muted)]">Get your key at <a href="https://www.semrush.com/api-analytics/" target="_blank" rel="noreferrer" className="vt-link">semrush.com</a></p>
          </div>
        )}
        <div className="flex gap-3 pt-2">
          <button onClick={onClose} className="vt-btn-secondary flex-1">Cancel</button>
          <button onClick={handleSave} disabled={saving} className="vt-btn-primary flex-1">{saving ? "Saving…" : "Save Key"}</button>
        </div>
      </div>
    </div>
  );
...
function Stat({ label, value, tone }: { label: string; value: number; tone: "emerald" | "amber" | "muted" }) {
  const color = tone === "emerald" ? "text-emerald-400" : tone === "amber" ? "text-amber-400" : "text-[color:var(--muted)]";
  return (
    <div className="vt-card p-4">
