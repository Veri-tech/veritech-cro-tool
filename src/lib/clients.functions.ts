...
const SetClientNoteDateInput = z.object({
  clientId: z.string().uuid(),
  noteDate: z.string().nullable(),
});
export const setClientNoteDate = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: unknown) => SetClientNoteDateInput.parse(d))
  .handler(async ({ data, context }) => {
    ...
    const { error } = await supabase
      .from("clients")
      .update({ note_date: data.noteDate })
      .eq("id", data.clientId)
