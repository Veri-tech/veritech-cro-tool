...
// ---------- Agency: get GA4 properties for a connected client ----------
...
// ---------- Agency: save selected GA4 property and GSC site ----------
export const saveClientOAuthSelection = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (data.ga4PropertyId) {
      ...
      await supabaseAdmin
        .from("client_integrations")
        .update({ manual_credentials: encrypted, auth_method: "oauth", status: "active" })
        .eq("client_id", data.clientId)
        .eq("provider", "google");
    }
